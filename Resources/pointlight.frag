...
void main(void)
{
    ...
    float offset = 0;//0.5 + intercept * dx_2;
    float lightDist = length(lightVec)*3.6;
	...
	vec3 diffuse = falloffTerm *u_lightColor ;

	vec4 tvec;
	//if(normDotPosToLight > 0.5)
	if(falloffTerm > 0.5)
	{
		tvec = vec4(texColor.rgb * (diffuse + u_ambientColor), texColor.a);
	}
	else
	{
		tvec = vec4(0,0,0,texColor.a);
	}

	gl_FragColor = tvec;//vec4(tvec, texColor.a);
}